...
enum abstract Direction(Bool) from Bool to Bool
{
	var FADE_OUT = true;
	var FADE_IN = false;
}
...
class Fade
{
	...
	**/
	 ...
	 * @since 1.0.0
	**/
	public var direction(default, null):Direction = FADE_OUT;
	...
	**/
	 ...
	 * @since 1.0.0
	**/
	private var _onUpdate:CameraFadeCallback = null;
	...
	**/
	 ...
	 * @since 1.0.0
	 *
	 * @param direction - The direction of the fade. `true` = fade out (transparent to color), `false` = fade in (color to transparent)
	 ...
	 */
	public function start(direction:Direction = FADE_OUT, duration:Int = 1000,
			red:Int = 0, green:Int = 0, blue:Int = 0, force:Bool = false,
			?callback:CameraFadeCallback, ?context:Any)
	{
		if (!force && isRunning)
		{
			return camera;
		...
		this.green = green;
		this.blue = blue;
		alpha = direction ? MathConst.CLOSEST_TO_ZERO : 1;
	...
	**/
	public function update(time:Int, delta:Float)
	{
		...
		if (_onUpdate != null)
		{
			callOnUpdate(_onUpdateScope, camera, progress);
		}

		if (_elapsed < duration)
		{
			alpha = (direction) ? progress : 1 - progress;
		}
		else
		{
			alpha = (direction) ? 1 : 0;
	...
	**/
	...
	private inline function callOnUpdate(context:Any, camera:Camera, progress:Float):Void
	{
		#if js
		if (_onUpdateScope != null)
		{
			(cast _onUpdate : js.lib.Function).call(_onUpdateScope, camera, progress);
		}
		else
		#end
		{
