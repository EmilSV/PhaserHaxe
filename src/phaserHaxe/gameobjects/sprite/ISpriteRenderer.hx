...
import phaserHaxe.gameobjects.components.IPipeline;
...
@:phaserHaxe.Mixin(phaserHaxe.gameobjects.sprite.ISpriteRenderer.SpriteRendererMixin)
...
{
	public static inline function renderCanvas<T:ISpriteRenderer>(self:T,
	...
	public static inline function renderWebGL<T:ISpriteRenderer & IPipeline>(self:T,
			...
			camera:Camera, parentMatrix:TransformMatrix):Void
	{
		self.pipeline.batchSprite(src, camera, parentMatrix);
	}
}
...
final class SpriteRendererMixin extends GameObject implements ISpriteRenderer implements IPipeline
{
	...
	**/
	public function renderWebGL(renderer:WebGLRenderer, src:Sprite,
			...
			parentMatrix:TransformMatrix):Void
	{
		SpriteRendererImplementation.renderWebGL(this, renderer, src, interpolationPercentage, camera, parentMatrix);
	}
